...
btn_newnote = Button(root, bg="#ffa2a2", text="+ New note", command=lambda: open_new_note())
btn_newnote.config()
btn_newnote.grid(row=1, sticky=W)

btn_shopping = Button(root, bg="#ffe4a2", text="Shopping", command=lambda: open_list("Shopping"))
btn_shopping.config()
btn_shopping.grid(row=2, sticky=E + W)

btn_todo = Button(root, bg="#f5ffa2", text="To do", command=lambda: open_list("To do"))
btn_todo.config()
